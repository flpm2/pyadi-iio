...
@pytest.mark.parametrize("dds_scale, min_rssi, max_rssi", [(0, 35, 60), (0.9, 0, 22)])   #### previous way of paramterizing did not work, complaining for missing max_rssi
...
@pytest.mark.iio_hardware(hardware)
@pytest.mark.parametrize("classname", [(classname)])
@pytest.mark.parametrize("channel", [0, 1]) #, 2, 3]) ### ZCU102 - ADRV9009 TX has two channels 
@pytest.mark.parametrize(
    ...
    [
        dict(
            ...
            calibrate_rx_qec_en=1,
            calibrate_tx_qec_en=1,
            #calibrate=1,  ### causes I/O Error when enabled
        ),
        dict(
            trx_lo=3000000000,
            gain_control_mode_chan0="slow_attack",
